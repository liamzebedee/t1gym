...
function getData(glucoseFeed, fromTo, events, model) {
  ...
  // Run simulation.
  const intoFuture = 0
  let predicted = Model.simulate(
    observed1,
    intoFuture,
    events.map(eventToFunction),
    model
  )
...
const Graph = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const totalInsulin = events
        .filter((x) => x.type == "insulin")
        .reduce((prev, curr) => {
      ...
      <Flex align="center">
        ...
        <Stack shouldWrapChildren isInline>
          <Select
            ...
            onChange={(ev) => loadExperiment(ev.target.value)}
          >
          ...
          <Button size="sm" variantColor="green" onClick={saveExperiment}>
            Save
          </Button>
        </Stack>
      </Flex>
      ...
      <Flex align="center">
        ...
        <Flex flexGrow={1} align="right" flexDirection="column" align="top">
          ...
          <Flex align="right" flexDirection="row">
            <span>
              <b>Start BG</b>: {(stats.startBG || 0).toFixed(1)}mmol
            </span>
            <Box paddingRight="5"></Box>
            <span>
              <b>End BG</b>: {(stats.endBG || 0).toFixed(1)}mmol
            </span>
            <Box paddingRight="5"></Box>
            <span>
              <b>Δ BG</b>: {(stats.deltaBG || 0) > 1 ? "+" : "-"}
              {(stats.deltaBG || 0).toFixed(1)}mmol
            </span>
            <Box paddingRight="5"></Box>
            <span>
              <b>Total insulin</b>: {stats.totalInsulin.toFixed(2)}U
            </span>
            <Box paddingRight="5"></Box>
            <span>
              <b>Total carbs</b>: {stats.totalCarbs}g
            </span>
          </Flex>
          ...
          <Stack shouldWrapChildren isInline>
            <FormControl>
              <FormLabel htmlFor="carb-sensitivity">
                🍎 Carb. sensitivity (15g : x mmol)
              </FormLabel>
              <NumberInput
                id="carb-sensitivity"
                size="sm"
                defaultValue={carbSensitivity}
                precision={2}
                step={0.01}
                onChange={debouncedOnChangeNumberInput((x) =>
                  setCarbSensitivity(parseFloat(x))
                )}
              >
                <NumberInputField />
                <NumberInputStepper>
            ...
            <FormControl>
              <FormLabel htmlFor="insulin-sensitivity">
                💉 Insulin sensitivity (1U : x mmol)
              </FormLabel>
              <NumberInput
                size="sm"
                defaultValue={insulinSensitivity}
                precision={1}
                min={-10}
                step={0.1}
                max={0}
                onChange={debouncedOnChangeNumberInput((x) =>
                  setInsulinSensitivity(parseFloat(x))
                )}
              >
                <NumberInputField />
                <NumberInputStepper>
...
const testing = compose(
  functions.foodDigestionEffect(functions.foodDigestion("carbs", 40, 51 / 100)),
...
const FunctionPlots = () => {
  return (
    <>
      ...
      />
      <FunctionPlot
        title="Exercise (80% intensity, duration 30mins)"
        id="exercise"
        fn={exerciseFn}
        duration={2 * HOUR}
      />
      <FunctionPlot
        title="Fiasp insulin active"
        id="insulin"
        fn={insulinActive}
        duration={7 * HOUR}
      />
      <FunctionPlot
        title="Food digestion (30g - Carbs)"
        id="insulin"
        fn={foodDigestionCarbs}
        duration={7 * HOUR}
      />
      <FunctionPlot
        title="Food digestion (30g - Protein)"
        id="insulin"
        fn={foodDigestionProtein}
        duration={7 * HOUR}
      />
      <FunctionPlot
        title="Basal rate"
