...
import {
  ...
  usePromiseLoadingStateWithError,
} from "../helpers"
...
async function _authenticate() {
  ...
  const res = await fetch(`/api/auth/beta?code=${code}`).then((res) =>
    res.json()
  )
  if (res.error) {
  ...
  try {
    ...
    throw new Error(
      "Error signing in with Firebase: " + error.code + " " + error.message
    )
  }
}
...
export default () => {
  return (
    <AppWrapper authenticateUser={false}>
      <SignUp />
    </AppWrapper>
  )
}
