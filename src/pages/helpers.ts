...
export function parseEvents(
  eventsString,
  insulinPumpSettings: InsulinPumpModel
): Event[] {
  ...
  return eventsString
    ...
    .map((line) => {
      ...
      if (type == "bolus") {
        ...
        // get bolus ratio
        const insulinAmount =
          carbAmount / insulinPumpSettings.getBolusRatio(date.c.hour)
      ...
      if (type == "correct") {
        ...
        const insulinAmount =
          reduceLevelBy / insulinPumpSettings.getCorrectionRatio(date.c.hour)
...
export function eventToFunction(event) {
  switch (event.type) {
    ...
    case "insulin": {
      ...
      return compose(
        functions.insulinGlucoseEffect(functions.fiaspInsulinActive(amount)),
        functions.beginsAfter({
          start,
        })
      )
