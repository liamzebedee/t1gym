...
export function calcStats(sgvData, userProfile) {
  ...
  data = data.filter(function (item) {
    ...
    return seen.includes(item.displayTime)
      ? false
      : (seen[item.displayTime] = true)
  })
  ...
  for (let i = 0; i <= data.length - 2; i++) {
    ...
    var timeDelta =
      nextEntry.displayTime.getTime() - entry.displayTime.getTime()
  ...
  for (let i = 1; i <= glucose_data.length - 2; i++) {
    ...
    let timeDelta =
      nextEntry.displayTime.getTime() - entry.displayTime.getTime()
    let timeDelta2 =
      entry.displayTime.getTime() - prevEntry.displayTime.getTime()
  ...
  result.Normal.readingspct = (
    100 -
    result.Low.readingspct -
    result.High.readingspct
  ).toFixed(1)
  ...
  const hba1c = (
    Math.round((10 * (ss.mean(mgDlBgs) + 46.7)) / 28.7) / 10
  ).toFixed(1)
  return {
    PGS,
    GVI,
