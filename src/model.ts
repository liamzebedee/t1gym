...
`
  .split("\n")
  .filter((x) => !!x)
  .map((line) => line.match(/\S+/g).map((x) => parseFloat(x)))
...
export const functions = {
  exercise(intensity) {
    return (u) => (intensity * -6) / ((60 * MINUTE) / u) // burn -6mmol every 60 minutes of exercise
  },
  ...
  basalEffect(rate) {
    return (u) => {
      ...
      for (let i = 0; i < u; i += MINUTE) {
        ...
        g += functions.insulinGlucoseEffect(
          functions.fiaspInsulinActive(rate / 60)
        )(u + i)
      }
      return g
    }
...
class Model {
   ...
   */
  static simulate(
    observed: GlucoseFeed,
    intoFuture: number = 0,
    functionalEffects: Function[],
    model: BodyMetabolismModel
  ): GlucoseFeed {
    if (!observed.length) {
      return []
    }
    ...
    for (; date <= until + intoFuture; date += STEP_SIZE) {
      ...
      const functionalSgv = functionalEffects
        .map((f) => f(date))
        .reduce((prev, curr) => prev + curr, sgv)
