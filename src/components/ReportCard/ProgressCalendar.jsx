      ...
      <g transform={`translate(${margin.left}, ${margin.top})`}>
        {_.range(NUM_DAYS_TO_DISPLAY).map((dayIdx, i, daysToDisplay) => {
          ...
          if (datum && datum.stats) {
            ...
            pgs = (
              <g transform={`translate(65,80)`}>
                ...
                <text textAnchor="middle" class={styles.pgsLabel} dy=".3em">
                  {PGS.toFixed(0)}
                </text>
              </g>
            )
          ...
          return (
            <g
              key={i}
              transform={`translate(${ROW_WIDTH * (i % 7)}, ${
                ROW_HEIGHT * Math.floor(i / 7)
              } )`}
              className={`${styles.day} ${
                dateKey === selectedDay && styles.active
              }`}
              width={ROW_WIDTH}
              height={ROW_HEIGHT}
