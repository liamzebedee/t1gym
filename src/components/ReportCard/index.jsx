...
function dataToDayByDay(longitudalData) {
  ...
  const buckets = _.groupBy(dataSortedByDate, (el) => {
    const day = DateTime.fromMillis(el.date).set({
      hour: 0,
      minute: 0,
      second: 0,
      millisecond: 0,
...
const Blip = ({ pgs }) => {
  const r = 25 / 2
  return (
    <svg style={{ display: "inline" }} width={r * 2} height={r * 2}>
...
export const ReportCard = ({ userProfile = PROFILE }) => {
  ...
  async function load() {
    ...
    const longitudalData = await fetch(
      `/api/bgs/longitudal?${queryString.stringify(params)}`
    ).then((res) => res.json())
  ...
  return (
    <>
      <p>
        ...
        {(userProfile.targetRange.bgTargetBottom / 18).toFixed(1)} -{" "}
        {(userProfile.targetRange.bgTargetTop / 18).toFixed(1)} mmol/L.
      </p>
      <br />
      <Flex direction="row">
