...
const Annotation = ({ startTime, endTime, tags, notes, active }) => {
  ...
  return (
    <>
      <TableCell>
        <Text>
          {active ? <b>{start.toFormat("t")}</b> : start.toFormat("t")}
        </Text>
      </TableCell>
      <TableCell>
        {tags.join(", ")}
...
function isInsulinTreatment(treatment) {
  return (
    (treatment.eventType == "Meal Bolus" && treatment.insulin != null) ||
...
export const Annotator = (props) => {
  ...
  const [
    ...
    onHoverAnnotation,
    onSelectAnnotation,
  ] = useHoverPickSelector()

  const day = getStartOfDayForTime(data[0].date)

  return (
    <Box pb={10} pt={10} borderWidth="1px" shadow="xs">
          ...
          <Stack flexGrow={1} spacing={8}>
            <Box>
              ...
              <Table>
                ...
                <TableBody>
                  {_.sortBy(annotations, ["startTime"]).map((x, i) => (
                    <TableRow
                      key={x.id}
                      bg={i % 2 == 0 ? "white" : "gray.50"}
                      className={`${styles.annotation} ${
                        selectedAnnotation === x.id && styles.active
                      }`}
            ...
            <Box p={2} shadow="xs" borderWidth="1px">
              {(function () {
                if (brush == null)
                  return <b>Highlight your chart to add notes</b>
                else {
                  return (
                    <>
          ...
          <Chart
            ...
            annotations={
              previewedAnnotation != null && [
                _.find(annotations, { id: previewedAnnotation }),
              ]
            }
            events={treatments}
