...
export const Analyse = () => {
  ...
  async function getBGData_() {
    ...
    const data = await fetch(
      `/api/bgs/daily?${queryString.stringify(params)}`
    ).then((res) => res.json())
    setBgs(data)
  }
  ...
  return (
    <Box p={5} boxShadow="lg">
      <Text fontSize="xl" pb={5}>
        <b>Label and understand your patterns</b>. Start by highlighting
        directly on your chart, and you can begin to add notes and build a
        richer understanding of your day.
      </Text>

      {loadingBGData && (
        <>
          <span>
            <CircularProgress isIndeterminate size="sm" color="green" /> Loading
            BG's from Nightscout...
          </span>
        </>
      )}
